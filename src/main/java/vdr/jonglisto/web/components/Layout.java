...
public class Layout extends BaseComponent {
	...
	public List<VDRView> getViews() {
		if (type == null) {
			...
			type = Type.View;
			currentVdrView = configuration.getConfiguredViews().values().stream().filter(s -> s.getType() == type).sorted().findFirst().get();
		}
		
		return configuration.getConfiguredViews().values().stream().filter(s -> s.getType() == type).sorted().collect(Collectors.toList());
	}
	
	public void onSelectView(String displayName) {
		currentVdrView = configuration.getConfiguredViews().values().stream().filter(s -> s.getDisplayName().equals(displayName)).findFirst().get();
