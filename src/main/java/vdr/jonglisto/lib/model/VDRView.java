...
public class VDRView implements Comparable<VDRView> {
    ...
    public Optional<String> getChannelVdr() {
        try {
            return Optional.of(channelVdr.stream().filter(s -> config.testRestfulApi(s.getIp(), s.getRestfulApiPort()))
    ...
    public Optional<String> getRecordingVdr() {
        try {
            return Optional.of(recordingVdr.stream()
                    .filter(s -> config.testRestfulApi(s.getIp(), s.getRestfulApiPort())).findFirst().get().getUuid());
        } catch (Exception e) {
            return Optional.empty();
        }
    }
    ...
    public String toString() {
        return "VDRView [config=" + config + ", displayName=" + displayName + ", channelVdr=" + channelVdr
                + ", timerVDR=" + timerVDR + ", headVDR=" + headVDR + ", recordingVdr=" + recordingVdr + ", type="
                + type + "]";
    }
