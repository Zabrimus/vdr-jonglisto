...
public class SearchTimer {
    ...
    public List<String> getCategory() {
        ...
        if (StringUtils.isNotEmpty(category)) {
            return Arrays.stream(category.split(",")).map(s -> s.replaceAll("^'", "").replaceAll("'$", ""))
    ...
    public List<String> getGenre() {
        ...
        if (StringUtils.isNotEmpty(genre)) {
            return Arrays.stream(genre.split(",")).map(s -> s.replaceAll("^'", "").replaceAll("'$", ""))
                    .collect(Collectors.toList());
        } else {
            return Collections.emptyList();
        }
    ...
    public String toString() {
        return "SearchTimer ["
                + dbData.keySet().stream().sorted().map(s -> s + "->" + dbData.get(s)).collect(Collectors.joining(", "))
                + "]";
    }
}
