...
public class EpgImageServiceImpl extends ServiceBase implements EpgImageService {
	...
	public byte[] getSeriesMediaImage(long seriesId, long seasonNumber, long episodeId, int mediaType, long actorId) {
		...
		try (Connection con = sql2o.open()) {
			...
			result = con.createQuery("select media_content from series_media where series_id = :seriesId and season_number = :seasonNumber and episode_id = :episodeId and media_type = :mediaType and actor_id = :actorId ") //
	...
	public byte[] getMovieMediaImage(long movieId, int mediaType, long actorId) {
		...
		try (Connection con = sql2o.open()) {
			...
			result = con.createQuery("select media_content from movie_media where movie_id = :movieId and media_type = :mediaType and actor_id = :actorId ") //
