...
public class Configuration {
	...
	private boolean syncMapConfirmed = false;
	...
	public boolean isSyncMapConfirmed() {
		return syncMapConfirmed;
	}

	public void setSyncMapConfirmed(boolean syncMapConfirmed) {
		this.syncMapConfirmed = syncMapConfirmed;
	}
	...
	public void sendWol(String uuid) {
		...
		if (v != null) {
			log.info("Wake on Lan is not yet implemented: {}", v.getDisplayName());
		} else {
	...
	private void initConfiguration(String pathname) {
		...
		try {
			...
			syncMapConfirmed = Boolean.valueOf((String) config.get("areYouSureToUseSyncMap"));
	...
	private VDR addinfo(VDR vdr, ArrayList<Map<String, Object>> vdrConfig, Map<String, String> aliases) {
		// Uuid suchen
		Optional<Map<String, Object>> cfg = vdrConfig.stream().filter(s -> vdr.getUuid().equals(aliases.get(s.get("uuid")))).findFirst();
	...
	private void executeMysqlScript(String resource) {
		...
		try {
			 sql = Files.readAllLines(Paths.get(this.getClass().getResource(resource).toURI()), Charset.defaultCharset()).stream().collect(Collectors.joining("\n"));
		} catch (Exception e) {
